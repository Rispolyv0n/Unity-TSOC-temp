...
public class GetFavShopList : MonoBehaviour {
    ...
    static public bool ifStoreOpen(string shopID) {
        foreach (GamingInfo.storeInfoWithID item in GamingInfo.storeInfo) {
            if (shopID == item.id)
            {
                ...
                // catch exception?
                if (item.info.openTime[dayOfWeek].open == true)
                {
                    foreach (GamingInfo.period periodItem in item.info.openTime[dayOfWeek].timePeriod)
                    {
